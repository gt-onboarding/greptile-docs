---
title: AI Task Enricher
description: 'Use the Greptile API to magically add instructions to every new ticket with codebase context.'
---
...
Today, we will integrate Greptile with your task manager (in this example, [Linear](linear.app)) so every time a new ticket is created, Greptile will add a comment with context from the codebase including specific file paths relevant to the task. 
...
<Steps>
    <Step title="Create a Zapier account">
        Go to [zapier.com](zapier.com) and create an account. 
    </Step>
...
```json
{
   ...
   "repositories":[
      ...
      {
         "repository":"tabnam-inc/tabnam-api",
         "branch":"main"
      }
   ]
}
...
Remember at add `"Bearer"` to the `Authorization` header value. 
...
That's it! Test the Zap and with 20-25 seconds of creating a new ticket in Linear, you should see a comment from Greptile adding helpful context!
